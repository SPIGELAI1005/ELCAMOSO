...
export function AudioPerfMeter({ perf, active }: { perf: AudioPerf | null; active: boolean }) {
  if (!active || !perf) {
    return (
  ...
  return (
    <div className="mt-6 border border-border p-5">
      <p className="text-[11px] tracking-[0.34em] text-muted-foreground uppercase">Audio monitor</p>
      ...
      <p className="mt-4 text-xs text-muted-foreground">
        Load is the share of each animation frame spent updating layers and environments. Stalls
        rise when the audio clock lags the display clock.
      </p>
    </div>
  );
}
