...
function demoSessionConfig(settings: ElcamosoSettings) {
  ...
  return {
    ...
    profileGain: getProfileGain(settings, profileId),
    tuning: getTuning(settings, profileId),
...
function DemoDrive() {
  ...
  const syncDemoSession = useCallback((next: ElcamosoSettings) => {
    getSession().syncConfig(demoSessionConfig(next));
  }, []);

  const onProfileSelect = useCallback(
    (profileId: string) => {
      const next = { ...settings, profileId };
      update({ profileId });
      syncDemoSession(next);
    },
    [settings, syncDemoSession, update],
  );

  const onEnvironmentChange = useCallback(
    (environmentId: string) => {
      const next = { ...settings, environmentId };
  ...
  const onGasEngage = useCallback(
    (throttle: number) => {
      getSession().primeAudioFromUserGesture();
      if (!active) {
        void getSession().startDemo({
      ...
      setControls({ throttle, regen: 0 });
    },
    [active, controls.accel, controls.selector, setControls],
  );
      ...
      <div className="mx-auto w-full max-w-3xl px-6 pt-16 pb-28 sm:px-10">
        <div className="max-w-3xl">
          <div className="relative w-fit">
            <h1 className="text-3xl font-light text-muted-foreground">Feel the Drive.</h1>
            <span
              aria-hidden
              className="pointer-events-none absolute top-[52%] left-0 h-[2px] w-full -translate-y-1/2 bg-[#e53935]"
            />
          </div>
          <p className="mt-3 text-3xl font-light leading-snug text-foreground">
        ...
        <section id="feel" className="mt-10 scroll-mt-24">
          <div className="overflow-hidden rounded-2xl border border-border bg-[#0A0A0A]/40">
...
function Readout({
  ...
  return (
    <div>
      ...
      <dt className="mt-1 text-[9px] tracking-[0.24em] text-muted-foreground uppercase">{label}</dt>
    </div>
  );
}
