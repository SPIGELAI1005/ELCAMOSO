...
export function validateRelayUpgrade(params: {
  ...
  const session = validateRelayToken(params.sessionId, params.token);
  if (!session) {
...
export function onRelayOpen(peer: RelayPeerLike) {
  attachRelayPeer(peer.context.sessionId, peer.context.role, {
    id: peer.id,
